...
- Rename display title to **shfmt WASM Formatter** and rewrite the marketplace description and keywords to clearly distinguish this extension from the upstream `foxundermoon.shell-format` (per VS Marketplace policy).
- Replace the logo with an original design.
- Drop upstream-only boilerplate from the repo (CI/CD workflows, FUNDING, husky, typings, GIF assets) so the repo no longer mirrors the upstream project.

## 0.1.1 — 2026-04-20

- Update README and repo metadata to reflect the fork (new name, publisher, credits, upstream context).

## 0.1.0 — 2026-04-20

Initial release of the **Shell Format (WASM)** fork.

- Replace `shfmt` binary spawning with in-process WASM ([sh-syntax](https://github.com/un-ts/sh-syntax)), removing the binary download step entirely.
- Fix activation crash caused by missing `one_ini_bg.wasm` (upstream [#396](https://github.com/foxundermoon/vs-shell-format/issues/396)) by pinning `editorconfig` to the pure-JS 0.15.x line.
- Remove the obsolete `shellformat.path` setting.
- Keep full compatibility with `shellformat.flag` and `shellformat.useEditorConfig`.

---

## Upstream (foxundermoon.shell-format) history below
